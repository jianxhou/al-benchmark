...
Correction after auditing the arXiv v2 supplementary and the repository docs (see `literature/death_supplementary_audit.md`): finding 1 is partially documented — the absolute-distance metric and the logSixHumpCamel shift constants (a = 1.0316, b = 1e-4) are stated in the supplementary, though the abs-before-cummin order and the coarse yopt are not. Findings 2 and 3 remain undocumented; the supplementary affirms the unscaled MAD definition and repeats the one-sided test claim three times.
